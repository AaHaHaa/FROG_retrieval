...
while flag > 0
    ...
    denominator = sum(sum(clean_image.^2));
    xCenter = sum(sum(x.*clean_image.^2))/denominator;
    yCenter = sum(sum(y.*clean_image.^2))/denominator;
    D4SigmaXTmp = 4*sqrt( sum(sum((x-xCenter).^2.*clean_image.^2))/denominator );
    D4SigmaYTmp = 4*sqrt( sum(sum((y-yCenter).^2.*clean_image.^2))/denominator );
    
    % Increase the threshold for the next iteration
    threshold = threshold*1.5;
    
    % Check if the conditions are fulfilled
    if (D4SigmaXTmp/D4SigmaX > 0.98 && D4SigmaYTmp/D4SigmaY > 0.98) || ...
            threshold > max(image(:))
        flag = 0;
    end

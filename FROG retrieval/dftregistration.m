...
if isequal(class(buf1ft),'gpuArray')
    use_gpu = true;
else
    use_gpu = false;
end
...
% Partial-pixel shift
    ...
    CC(m+1-fix(m/2):m+1+fix((m-1)/2),n+1-fix(n/2):n+1+fix((n-1)/2)) = ...
        myfftshift(buf1ft.*conj(buf2ft));
    
    % Compute crosscorrelation and locate the peak 
    CC = ifft2(myifftshift(CC)); % Calculate cross-correlation
    [max1,loc1] = max(CC);
    [~,loc2] = max(max1);
...
if nargout > 1 && usfac > 0
    [nr,nc] = size(buf2ft);
    Nr = [0:ceil(nr/2)-1,-fix(nr/2):-1]; % ifftshift(-fix(nr/2):ceil(nr/2)-1)
    Nc = [0:ceil(nc/2)-1,-fix(nc/2):-1]; % ifftshift(-fix(nc/2):ceil(nc/2)-1)
...
kernc = exp((-1i*2*pi/(nc*usfac))*( [0:ceil(nc/2-1),-floor(nc/2):-1].' )*( (0:noc-1) - coff ));
kernr = exp((-1i*2*pi/(nr*usfac))*( (0:nor-1).' - roff )*( [0:ceil(nr/2-1),-floor(nr/2):-1] ));
out = kernr*in*kernc;

end

function A = myfftshift(A)

...
%       duration = ?; % ps
%       time_window = ?; % ps
%       omega = ifftshift(2*pi*(-N/2:N/2)/time_window); % 2*pi*THz
%       spectrum_amplitude = fft(E,1); % E is the electric field
...
function  chirp = Gaussian( duration,bandwidth,chirp_sign )
...
%       bandwidth: FWHM under frequency domain; 2*pi*THz
...
omega = ifftshift(omega,1)*1e-12;
spectrum_amplitude = ifftshift(spectrum_amplitude,1);
...
[~,~,transform_limited_FWHM] = calc_transform_limited( [],fftshift(ifft(spectrum_amplitude)),3,time );

if duration <= transform_limited_FWHM/1e3
    chirp = 0;
else
    ...
    guess_desired_chirp = Gaussian( duration,input_bandwidth,chirp_sign );

    % Add the chirp based on chirp_sign
    options = optimset('TolX',1e-20);
    find_optimal_chirp = @(C) find_fwhm( abs(C)*sign(chirp_sign),time,duration,omega,omega0,spectrum_amplitude_thresholded );
    stretching_chirp = fminsearch(find_optimal_chirp,guess_desired_chirp,options);
    chirp = abs(stretching_chirp)*sign(chirp_sign);
end
chirped_pulse = ifft(spectrum_amplitude.*exp(1i*chirp/2*(omega-omega0).^2));

end

function [difference,pulse_FWHM] = find_fwhm( C,time,duration,omega,omega0,spectrum_amplitude )
%FIND_FWHM

pulse = sum(abs(fftshift(ifft(spectrum_amplitude.*exp(1i*C/2*(omega-omega0).^2)),1)).^2,2);
